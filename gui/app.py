"""
Основной модуль приложения Crayfish AI Studio Pro - GUI и основная логика
"""
...
from models_manager import ModelTrainer, ModelOptimizer
...
try:
    ...
    class MockModule:
        ...
        def __getattr__(self, name):
            return type('Empty', (), {})()
    sys.modules['unittest'] = MockModule('unittest')
    sys.modules['unittest.mock'] = MockModule('unittest.mock')
    sys.modules['unittest.util'] = MockModule('unittest.util')
    sys.modules['unittest.case'] = MockModule('unittest.case')
    sys.modules['unittest.suite'] = MockModule('unittest.suite')
    sys.modules['unittest.loader'] = MockModule('unittest.loader')
    sys.modules['unittest.result'] = MockModule('unittest.result')
    sys.modules['unittest.runner'] = MockModule('unittest.runner')
    sys.modules['unittest.signals'] = MockModule('unittest.signals')
...
class ModernCrayfishDetector:
    def __init__(self, root):
        self.root = root
        self.root.title("Crayfish AI Studio Pro v2.0 (OBB Support)")
    ...
    def setup_matplotlib_style(self):
        ...
        rcParams['axes.facecolor'] = '#1a2b3c'
        rcParams['axes.edgecolor'] = '#00d4ff'
        rcParams['axes.labelcolor'] = 'white'
        rcParams['text.color'] = 'white'
        rcParams['xtick.color'] = 'white'
        rcParams['ytick.color'] = 'white'
        rcParams['grid.color'] = '#2a3b4c'
        rcParams['figure.facecolor'] = '#0f1a2b'
        rcParams['figure.edgecolor'] = '#0f1a2b'
    ...
    def create_quick_actions(self, parent):
        ...
        canvas = tk.Canvas(main_container, bg=self.colors['card_bg'], highlightthickness=0)
    ...
    def _fill_quick_actions_content(self):
        ...
        actions = [
            ...
            ("ДООБУЧИТЬ МОДЕЛЬ", self.finetune_model_ui),
            ("ОПТИМИЗИРОВАТЬ МОДЕЛЬ (выбрать файл)", self.optimize_model_ui),
            ("Сравнить оптимизации", self.compare_optimizations),
    ...
    def create_telegram_bot_ui(self, parent):
        ...
        tk.Label(telegram_frame, text="Токен бота (Ctrl+V):", 
                font=('Arial', 9),
                fg=self.colors['text_secondary'],
    ...
    def create_image_viewer(self, parent):
        ...
        optimization_tab = ttk.Frame(self.notebook)
        self.notebook.add(optimization_tab, text="Оптимизация")
        self.create_optimization_tab(optimization_tab)
        
        training_tab = ttk.Frame(self.notebook)
        self.notebook.add(training_tab, text="Дообучение")
    ...
    def create_training_tab(self, parent):
        training_frame = tk.Frame(parent, bg=self.colors['card_bg'])
        training_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        tk.Label(training_frame, text="Дообучение модели YOLO", 
                font=('Arial', 14, 'bold'),
                fg=self.colors['text_primary'],
                bg=self.colors['card_bg']).pack(pady=10)
        
        instructions = tk.Text(training_frame, height=6, width=50,
                             bg='#1e2b3a', fg=self.colors['text_primary'],
                             font=('Arial', 10), wrap=tk.WORD)
        instructions.pack(fill=tk.X, pady=10, padx=10)
        
        instructions_text = """ИНСТРУКЦИЯ ПО ДООБУЧЕНИЮ:

1. Структура датасета должна соответствовать формату YOLO:
   • dataset.yaml в корне
   • Папки train/images, train/labels
   • Папки val/images, val/labels
   
2. Пример dataset.yaml:
...
3. Нажмите кнопку ниже для выбора датасета"""
        
        instructions.insert(1.0, instructions_text)
        instructions.config(state=tk.DISABLED)
        
        buttons_frame = tk.Frame(training_frame, bg=self.colors['card_bg'])
        buttons_frame.pack(fill=tk.X, pady=10)
        
        tk.Button(buttons_frame, text="Выбрать датасет для дообучения",
                 bg=self.colors['primary'], fg='white',
                 font=('Arial', 11),
                 command=self.select_training_dataset).pack(fill=tk.X, pady=5)
        
        tk.Button(buttons_frame, text="Запустить дообучение",
                 bg=self.colors['accent'], fg='white',
                 font=('Arial', 11, 'bold'),
                 command=self.start_finetune_training).pack(fill=tk.X, pady=5)
        
        params_frame = tk.Frame(training_frame, bg=self.colors['card_bg'])
        params_frame.pack(fill=tk.X, pady=10)
        
        tk.Label(params_frame, text="Параметры обучения:", 
                font=('Arial', 11, 'bold'),
                fg=self.colors['text_primary'],
                bg=self.colors['card_bg']).pack(anchor=tk.W)
        
        epochs_frame = tk.Frame(params_frame, bg=self.colors['card_bg'])
        epochs_frame.pack(fill=tk.X, pady=5)
        
        tk.Label(epochs_frame, text="Количество эпох:", 
                font=('Arial', 10),
                fg=self.colors['text_secondary'],
                bg=self.colors['card_bg']).pack(side=tk.LEFT, padx=10)
        
        self.epochs_var = tk.StringVar(value="50")
        epochs_entry = tk.Entry(epochs_frame, textvariable=self.epochs_var,
                              font=('Arial', 10), width=10)
        epochs_entry.pack(side=tk.LEFT)
        
        imgsz_frame = tk.Frame(params_frame, bg=self.colors['card_bg'])
        imgsz_frame.pack(fill=tk.X, pady=5)
        
        tk.Label(imgsz_frame, text="Размер изображения:", 
                font=('Arial', 10),
                fg=self.colors['text_secondary'],
                bg=self.colors['card_bg']).pack(side=tk.LEFT, padx=10)
        
        self.imgsz_var = tk.StringVar(value="640")
        imgsz_entry = tk.Entry(imgsz_frame, textvariable=self.imgsz_var,
                             font=('Arial', 10), width=10)
        imgsz_entry.pack(side=tk.LEFT)
        
        results_frame = tk.Frame(training_frame, bg=self.colors['card_bg'])
        results_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        tk.Label(results_frame, text="Результаты обучения:", 
                font=('Arial', 11, 'bold'),
                fg=self.colors['text_primary'],
                bg=self.colors['card_bg']).pack(anchor=tk.W)
        
        self.training_results_text = tk.Text(results_frame, height=8, width=50,
                                           bg='#1e2b3a', fg=self.colors['text_primary'],
                                           font=('Arial', 9), wrap=tk.WORD)
        scrollbar = ttk.Scrollbar(results_frame, orient=tk.VERTICAL, command=self.training_results_text.yview)
        self.training_results_text.configure(yscrollcommand=scrollbar.set)
        
        self.training_results_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    
    def create_optimization_tab(self, parent):
        optimization_frame = tk.Frame(parent, bg=self.colors['card_bg'])
        optimization_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        tk.Label(optimization_frame, text="Оптимизация моделей YOLO", 
                font=('Arial', 14, 'bold'),
                fg=self.colors['text_primary'],
                bg=self.colors['card_bg']).pack(pady=10)
        
        info_text = tk.Text(optimization_frame, height=6, width=50,
                          bg='#1e2b3a', fg=self.colors['text_primary'],
                          font=('Arial', 10), wrap=tk.WORD)
        info_text.pack(fill=tk.X, pady=10, padx=10)
        
        info_content = """ДОСТУПНЫЕ МЕТОДЫ ОПТИМИЗАЦИИ:

1. INT8 КВАНТИЗАЦИЯ:
   • Сжатие модели до 8-битных целых чисел
   • Ускорение inference в 2-3 раза
   • Уменьшение размера на 60-70%
   
2. FP16 ПОЛУТОЧНОСТЬ:
   • Использование 16-битных чисел с плавающей точкой
   • Ускорение на GPU
   • Сохранение точности
...
4. ONNX КОНВЕРТАЦИЯ:
   • Конвертация в кросс-платформенный формат
   ...
   • Оптимизация для разных устройств"""
        ...
        buttons_frame = tk.Frame(optimization_frame, bg=self.colors['card_bg'])
        buttons_frame.pack(fill=tk.X, pady=10)
        
        tk.Button(buttons_frame, text="INT8 Квантизация",
                 bg=self.colors['primary'], fg='white',
                 font=('Arial', 10),
                 command=lambda: self.optimize_with_method('int8')).pack(fill=tk.X, pady=2)
        
        tk.Button(buttons_frame, text="FP16 Полуточность",
                 bg=self.colors['primary'], fg='white',
                 font=('Arial', 10),
                 command=lambda: self.optimize_with_method('fp16')).pack(fill=tk.X, pady=2)
        
        tk.Button(buttons_frame, text="Подрезка (Pruning)",
                 bg=self.colors['primary'], fg='white',
                 font=('Arial', 10),
                 command=lambda: self.optimize_with_method('prune')).pack(fill=tk.X, pady=2)
        
        tk.Button(buttons_frame, text="ONNX Конвертация",
                 bg=self.colors['primary'], fg='white',
                 font=('Arial', 10),
                 command=lambda: self.optimize_with_method('onnx')).pack(fill=tk.X, pady=2)
        
        tk.Button(buttons_frame, text="Сравнить все методы оптимизации",
                 bg=self.colors['accent'], fg='white',
                 font=('Arial', 11, 'bold'),
                 command=self.compare_optimizations).pack(fill=tk.X, pady=10)
        
        results_frame = tk.Frame(optimization_frame, bg=self.colors['card_bg'])
        results_frame.pack(fill=tk.BOTH, expand=True, pady=10)
        
        tk.Label(results_frame, text="Результаты оптимизации:", 
                font=('Arial', 11, 'bold'),
                fg=self.colors['text_primary'],
                bg=self.colors['card_bg']).pack(anchor=tk.W)
        
        self.optimization_results_text = tk.Text(results_frame, height=10, width=50,
                                               bg='#1e2b3a', fg=self.colors['text_primary'],
                                               font=('Arial', 9), wrap=tk.WORD)
        scrollbar = ttk.Scrollbar(results_frame, orient=tk.VERTICAL, command=self.optimization_results_text.yview)
        self.optimization_results_text.configure(yscrollcommand=scrollbar.set)
        
        self.optimization_results_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
...
• Угол поворота (для OBB моделей)
• ID особи (из БД)

НОВЫЕ ВОЗМОЖНОСТИ:
• Поддержка OBB (Oriented Bounding Box)
• Дообучение моделей YOLO
• 4 метода оптимизации
• Калибровка квадратом/прямоугольником
• Раздельные коэффициенты X/Y"""
    ...
    def load_available_models(self):
        model_paths = [
            ...
            "best.pt",
            "last.pt", 
            "training_results/train/weights/best.pt",
            "training_results/train2/weights/best.pt",
            "models/best.pt",
            "models/last.pt",
            "yolo11m.pt",
            "yolo11n.pt",
    ...
    def load_model_thread(self, model_path):
        def load():
            try:
                ...
                self.root.after(0, lambda: self.status_label.config(text=f"Модель загружена: {self.current_model}"))
                self.root.after(0, self.update_db_stats)
                
                self.model_trainer = ModelTrainer(model_path)
                
            except Exception as e:
                self.root.after(0, lambda: self.status_label.config(text=f"Ошибка загрузки модели: {str(e)[:50]}"))
    ...
    def update_ui_after_detection(self, processing_time, session_id, inference_time_ms):
        try:
            ...
            self.status_label.config(text=f"Детекция завершена ({processing_time:.2f}с, ML: {inference_time_ms:.1f}мс)")
                ...
                if file_path.endswith('.xlsx'):
                    try:
                        ...
                        messagebox.showwarning("Внимание", 
                                              ...
                                              "Пока что данные будут экспортированы в CSV.")
...
4. Получите токен
5. Вставьте токен ниже (можно использовать Ctrl+V)
...
/info - информация о боте"""
    ...
    def start_telegram_bot(self):
        ...
        try:
            ...
            if not self.telegram_bot.bot:
                messagebox.showerror("Ошибка", "Не удалось инициализировать бота. Проверьте токен!")
                return
    ...
    def stop_telegram_bot(self):
        if self.telegram_bot:
            ...
            messagebox.showinfo("Информация", "Telegram Bot остановлен")
        else:
            messagebox.showinfo("Информация", "Бот не запущен")
    
    def select_training_dataset(self):
        dataset_path = filedialog.askdirectory(
            title="Выберите папку с датасетом для дообучения",
            initialdir="."
        )
        ...
        if dataset_path:
            ...
            if not os.path.exists(yaml_path):
                if messagebox.askyesno("YAML файл не найден", 
                                      "Файл dataset.yaml не найден в датасете.\n"
                                      "Создать автоматически по шаблону?"):
                    self.create_dataset_yaml(dataset_path)
                else:
                    messagebox.showinfo("Информация", "Выберите датасет с правильной структурой YOLO")
                    return
            
            messagebox.showinfo("Успех", f"Датасет выбран:\n{dataset_path}\n\nТеперь нажмите 'Запустить дообучение'")
...
names: ['crayfish']"""
            ...
            messagebox.showinfo("Успех", f"Файл dataset.yaml создан:\n{yaml_path}\n\nТеперь можно запустить дообучение.")
            
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось создать YAML файл: {e}")
    
    def start_finetune_training(self):
        if not hasattr(self, 'training_dataset_path') or not self.training_dataset_path:
            messagebox.showwarning("Внимание", "Сначала выберите датасет для дообучения!")
            return
        ...
        try:
            ...
            if epochs <= 0 or imgsz <= 0:
                messagebox.showerror("Ошибка", "Параметры обучения должны быть больше 0!")
                return
        ...
        self.status_label.config(text=f"Запускаю дообучение на {epochs} эпохах...")
        
        threading.Thread(
            target=lambda: self.finetune_training_thread(self.training_dataset_path, epochs, imgsz),
            daemon=True
        ).start()
    
    def finetune_training_thread(self, dataset_path, epochs, imgsz):
        try:
            if not self.model_trainer:
                current_model_path = self.models.get(self.current_model)
                if not current_model_path:
                    self.root.after(0, lambda: messagebox.showerror("Ошибка", "Не удалось определить путь к текущей модели!"))
                    return
                self.model_trainer = ModelTrainer(current_model_path)
            
            training_results = self.model_trainer.finetune(dataset_path, epochs, imgsz)
            
            self.root.after(0, self.update_training_results_ui, training_results)
            self.root.after(0, lambda: self.status_label.config(
                text=f"Дообучение завершено за {training_results.get('training_time', 0):.1f}с"
            ))
            
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Ошибка", f"Ошибка дообучения: {e}"))
            self.root.after(0, lambda: self.status_label.config(text="Ошибка дообучения"))
    
    def update_training_results_ui(self, results):
        if results['status'] == 'COMPLETED':
            results_text = f"РЕЗУЛЬТАТЫ ДООБУЧЕНИЯ МОДЕЛИ\n\n"
            results_text += f"Статус: {results['status']}\n"
            results_text += f"Время обучения: {results['training_time']:.1f} секунд\n"
            results_text += f"Количество эпох: {results['epochs']}\n"
            results_text += f"Лучшая модель: {results['best_model']}\n"
            results_text += f"Последняя модель: {results['last_model']}\n\n"
            
            if 'metrics' in results and results['metrics']:
                metrics = results['metrics']
                results_text += f"Метрики качества:\n"
                results_text += f"• Precision: {metrics.get('precision', 0):.3f}\n"
                results_text += f"• Recall: {metrics.get('recall', 0):.3f}\n"
                results_text += f"• mAP50: {metrics.get('mAP50', 0):.3f}\n\n"
            
            results_text += f"Модель успешно дообучена и сохранена!"
            
            if messagebox.askyesno("Дообучение завершено", 
                                 "Хотите загрузить дообученную модель для использования?"):
                if os.path.exists(results['best_model']):
                    self.load_model_thread(results['best_model'])
            
        else:
            results_text = f"ОШИБКА ДООБУЧЕНИЯ\n\n"
            results_text += f"Статус: {results['status']}\n"
            results_text += f"Ошибка: {results.get('error', 'Неизвестная ошибка')}"
        
        self.training_results_text.config(state=tk.NORMAL)
        self.training_results_text.delete(1.0, tk.END)
        self.training_results_text.insert(1.0, results_text)
        self.training_results_text.config(state=tk.DISABLED)
        
        self.notebook.select(3)
    
    def finetune_model_ui(self):
        if not self.model:
            messagebox.showwarning("Внимание", "Сначала загрузите модель!")
            return
        
        self.select_training_dataset()
    
    def optimize_model_ui(self):
        file_path = filedialog.askopenfilename(
            title="Выберите модель YOLO для оптимизации",
            filetypes=[("YOLO models", "*.pt"), ("All files", "*.*")]
        )
        ...
        dialog = tk.Toplevel(self.root)
        dialog.title("Выбор метода оптимизации")
        dialog.geometry("400x300")
        dialog.configure(bg=self.colors['dark_bg'])
        
        tk.Label(dialog, text=f"Выбрана модель: {os.path.basename(file_path)}", 
                font=('Arial', 10),
                fg=self.colors['text_primary'],
                bg=self.colors['dark_bg']).pack(pady=10)
        
        tk.Label(dialog, text="ВЫБЕРИТЕ МЕТОД ОПТИМИЗАЦИИ", 
                font=('Arial', 14, 'bold'),
                fg=self.colors['primary'],
                bg=self.colors['dark_bg']).pack(pady=20)
        
        methods = [
            ("INT8 Квантизация", 'int8'),
            ("FP16 Полуточность", 'fp16'), 
            ("Подрезка (Pruning)", 'prune'),
            ("ONNX Конвертация", 'onnx')
        ]
        
        for text, method in methods:
            btn = tk.Button(dialog, text=text,
                          bg=self.colors['primary'], fg='white',
                          font=('Arial', 11),
                          command=lambda m=method, path=file_path: self.optimize_with_method(m, path, dialog))
            btn.pack(fill=tk.X, padx=50, pady=5)
        
        tk.Button(dialog, text="Отмена", 
                 bg=self.colors['secondary'], fg='white',
                 font=('Arial', 11),
                 command=dialog.destroy).pack(pady=20)
    
    def optimize_with_method(self, method, model_path, dialog=None):
        if dialog:
            dialog.destroy()
        
        self.status_label.config(text=f"Оптимизирую модель методом {method}...")
        
        threading.Thread(target=lambda: self.optimize_model_thread(method, model_path), daemon=True).start()
    
    def optimize_model_thread(self, method, model_path):
        try:
            optimization_results = self.model_optimizer.optimize(
                model_path,
                method=method
            )
            
            self.root.after(0, self.update_optimization_results_ui, optimization_results)
            self.root.after(0, lambda: self.status_label.config(
                text=f"Оптимизация {method} завершена. Сжатие: {optimization_results.get('compression_percent', 0):.1f}%"
            ))
            
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Ошибка", f"Ошибка оптимизации: {e}"))
            self.root.after(0, lambda: self.status_label.config(text="Ошибка оптимизации"))
    
    def update_optimization_results_ui(self, results):
        if results['status'] == 'COMPLETED':
            results_text = f"РЕЗУЛЬТАТЫ ОПТИМИЗАЦИИ ({results['method'].upper()})\n\n"
            results_text += f"Статус: {results['status']}\n"
            results_text += f"Метод: {results['method']}\n"
            results_text += f"Выходной файл: {results['output_path']}\n\n"
            results_text += f"РАЗМЕРЫ ФАЙЛОВ:\n"
            results_text += f"• Оригинальный: {results['original_size_mb']:.2f} MB\n"
            results_text += f"• Оптимизированный: {results['output_size_mb']:.2f} MB\n"
            results_text += f"• Коэффициент сжатия: {results['compression_ratio']:.2%}\n"
            results_text += f"• Процент сжатия: {results['compression_percent']:.1f}%\n\n"
            results_text += f"Оптимизация успешно завершена!"
        else:
            results_text = f"ОШИБКА ОПТИМИЗАЦИИ\n\n"
            results_text += f"Метод: {results['method']}\n"
            results_text += f"Ошибка: {results.get('error', 'Неизвестная ошибка')}"
        
        self.optimization_results_text.config(state=tk.NORMAL)
        self.optimization_results_text.delete(1.0, tk.END)
        self.optimization_results_text.insert(1.0, results_text)
        self.optimization_results_text.config(state=tk.DISABLED)
        
        self.notebook.select(1)
    
    def compare_optimizations(self):
        if not hasattr(self.model_optimizer, 'optimization_results') or not self.model_optimizer.optimization_results:
            messagebox.showinfo("Сравнение", "Сначала выполните оптимизацию хотя бы одним методом!")
            return
        
        results = self.model_optimizer.optimization_results
        
        comparison_text = "СРАВНЕНИЕ РЕЗУЛЬТАТОВ ОПТИМИЗАЦИИ\n\n"
        comparison_text += "=" * 60 + "\n\n"
        
        for method, result in results.items():
            if result['status'] == 'COMPLETED':
                comparison_text += f"МЕТОД: {method.upper()}\n"
                comparison_text += f"   Размер оригинала: {result['original_size_mb']:.2f} MB\n"
                comparison_text += f"   Размер оптимизированной: {result['output_size_mb']:.2f} MB\n"
                comparison_text += f"   Сжатие: {result['compression_percent']:.1f}%\n"
                comparison_text += f"   Файл: {os.path.basename(result['output_path'])}\n"
                comparison_text += "-" * 40 + "\n\n"
        ...
        for method, result in results.items():
            if result['status'] == 'COMPLETED' and result['compression_percent'] > best_compression:
                best_compression = result['compression_percent']
                best_method = method
        
        if best_method:
            comparison_text += f"ЛУЧШИЙ МЕТОД: {best_method.upper()}\n"
            comparison_text += f"   Сжатие: {best_compression:.1f}%\n\n"
        
        comparison_text += "=" * 60 + "\n\n"
        comparison_text += "РЕКОМЕНДАЦИИ:\n"
        comparison_text += "• Для максимального сжатия: INT8\n"
        comparison_text += "• Для баланса скорости и точности: FP16\n"
        comparison_text += "• Для совместимости: ONNX\n"
        comparison_text += "• Для уменьшения размера модели: Pruning"
        
        result_window = tk.Toplevel(self.root)
        result_window.title("Сравнение оптимизаций")
        result_window.geometry("600x500")
        result_window.configure(bg=self.colors['dark_bg'])
        
        text_widget = tk.Text(result_window, bg='#1e2b3a', fg='white',
                            font=('Arial', 10), wrap=tk.WORD)
